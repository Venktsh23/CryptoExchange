...
public class AccountService
{
        ...
        string userId, string currency, decimal amount)
    {
        ...
        var account = await _repo.DepositAsync(
            userId, currency.ToUpper(), amount);

        _logger.LogInformation(
            "DEPOSIT | User: {User} | {Amount} {Currency} | " +
            "New balance: {Balance}",
        ...
        decimal quantity)
    {
        ...
        if (side == "Buy")
        {
            ...
            var locked   = await _repo.TryLockFundsAsync(
                orderId, userId, quoteCcy, required);
            ...
            if (!locked)
            {
                ...
                return $"Insufficient {quoteCcy}. " +
                       $"Required: {required:F2}, " +
                       $"Available: {available:F2}";
            }

            _logger.LogInformation(
                "FUNDS LOCKED | User: {User} | {Amount} {Currency} " +
                "for order {OrderId}",
                userId, required, quoteCcy, orderId);
        }
        else // Sell
        {
            // Seller needs the crypto
            var locked = await _repo.TryLockFundsAsync(
                orderId, userId, baseCcy, quantity);
            ...
            if (!locked)
            {
                ...
                return $"Insufficient {baseCcy}. " +
                       $"Required: {quantity:F8}, " +
                       $"Available: {available:F8}";
            }
